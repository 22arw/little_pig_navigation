...
int main(int argc, char** argv)
{
    ...
    //Initiate subscribers
		...
		ros::Subscriber sub_gps = n.subscribe("/basic_laser_pig/simulation/fix", 100, filtered_gps_CB);
		ROS_INFO("Initiated collect_gps_waypoints node");

	// Initiate publisher to send end of node message
		ros::Publisher pubCollectionNodeEnded = n.advertise<std_msgs::Bool>("/basic_laser_pig/simulation/collection_status",100);

    //Read file path and create/open file
    	ros::param::get("/little_pig_navigation/coordinates_file", path_local);
		std::string path_abs =  ros::package::getPath("little_pig_navigation") + path_local;	
