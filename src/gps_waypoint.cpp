...
{
    path_abs = ros::package::getPath("little_pig_navigation") + path_local;
...
int main(int argc, char** argv)
{
    ...
    MoveBaseClient ac("/basic_laser_pig/simulation/move_base", true);
    ...
    ros::Publisher pubWaypointNodeEnded = n.advertise<std_msgs::Bool>("/basic_laser_pig/simulation/waypoint_following_status", 100);
    ...
    ROS_INFO("PLEASE HAVE RIGHT DATA: %s", path_local.c_str());
    ROS_INFO("PLEASE HAVE RIGHT DATA: %s", path_abs.c_str());
    ros::param::get("/little_pig_navigation/coordinates_file", path_local);
    ...
    for(iter = waypointVect.begin(); iter < waypointVect.end(); iter++)
    {
        ...
        else //set to current
        {
            ...
            longiNext = iter->second;
            final_point = true;
            ROS_INFO("FIRST - Received Latitude goal:%.8f", latiGoal);
            ROS_INFO("FIRST - Received longitude goal:%.8f", longiGoal);
        }

        ROS_INFO("Received Latitude goal:%.8f", latiGoal);
        ROS_INFO("Received longitude goal:%.8f", longiGoal);

        ROS_INFO("Received Latitude NEXT:%.8f", latiNext);
        ROS_INFO("Received longitude NEXT:%.8f", longiNext);

        //Convert lat/long to utm:
        UTM_point = latLongtoUTM(latiGoal, longiGoal);
        UTM_next = latLongtoUTM(latiNext, longiNext);

        ROS_INFO("UTM POINT:%.8f", UTM_point);
        ROS_INFO("UTM_NEXT:%.8f", UTM_next);

        //Transform UTM to map point in odom frame
        map_point = UTMtoMapPoint(UTM_point);
        map_next = UTMtoMapPoint(UTM_next);

        ROS_INFO("MAP_POINT:%.8f", map_point);
        ROS_INFO("MAP_NEXT:%.8f", map_next);
        ...
        if(ac.getState() == actionlib::SimpleClientGoalState::ACTIVE){
            ROS_INFO("GOAL IN PROGRESS - CURRENTLY ACTIVE");
        }
        //Wait for result
